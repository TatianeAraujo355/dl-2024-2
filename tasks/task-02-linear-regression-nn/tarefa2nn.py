...
def solve_linear_regression_nn(X_train, y_train, epochs=100, learning_rate=0.01):
    ...
    ### START CODE HERE ###
    # Reshape the inputs to match the neural network's expected input shape
    X_train = X_train.reshape(-1, 1)  # Shape (n_samples, 1)
    y_train = y_train.reshape(-1, 1)  # Shape (n_samples, 1)
    
    # Build the model: a simple neural network with one neuron (no activation function)
    model = keras.Sequential([
        keras.layers.Dense(1, input_dim=1, use_bias=True)  # Single neuron with bias
    ])
